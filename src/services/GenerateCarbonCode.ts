...
export async function generateCarbonCode({
  code,
  id,
}: GenerateCarbonCodeParams) {
  const data = {
    code: `${code}`,
  ...
  try {
    const response: AxiosResponse = await axios.post(
      `http://${env.CARBONARA_HOST}:3000/api/cook`,
      data,
      {
    ...
    img.write(
      path.resolve(process.cwd(), `${env.PATH_RESULTS}/${id}/code.png`)
    );
  } catch (error) {
    console.error(error);
  }
